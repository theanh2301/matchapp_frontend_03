...
class DashboardService {
  ...
  Future<DashboardResponse> getDashboardData(int userId, DateTime date) async {
    ...
    try {
      // Đã thêm headers chứa token và timeout tại đây
      ...
      if (response.statusCode == 200) {
        ...
        // Có thể thêm log cho 403 để dễ debug sau này
        print("❌ Lỗi 403 (Forbidden): ${response.body}");
        throw Exception("Không có quyền truy cập. Vui lòng kiểm tra lại token (Lỗi 403).");
      } else if (response.statusCode == 404) {
        print("❌ Lỗi 404 (Not Found): ${response.body}");
        throw Exception("Không tìm thấy dữ liệu hoặc API không tồn tại (Lỗi 404).");
      ...
      // PHÂN LOẠI LỖI MẠNG VÀ LỖI PARSE CODE
      if (e.toString().contains('SocketException')) {
        throw Exception("Không có kết nối mạng hoặc Server Spring Boot chưa bật.");
      } else if (e.toString().contains('TimeoutException')) {
        throw Exception("Kết nối tới server quá hạn. Vui lòng thử lại.");
      } else if (e.toString().contains('FormatException')) {
        throw Exception("Lỗi parse JSON. Server trả về dữ liệu không đúng chuẩn.");
      } else {
